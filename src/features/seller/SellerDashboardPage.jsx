...
import { Package, TrendingUp, Clock, Truck, LifeBuoy } from 'lucide-react'
...
export default function SellerDashboardPage() {
  const { ordersStore, ticketsStore } = useAgents()
  ...
  const reclamosPendientes = ticketsStore.filter((t) => t.needsHumanReview).length
  ...
  return (
    <div className="container">
      ...
      <div className={styles.linksRow}>
        ...
        <Link to="/vendedor/tickets" className={styles.linkCard}>
          <LifeBuoy size={20} />
          <div><strong>Soporte y HITL</strong><p>Revisa y aprueba propuestas de resolución de reclamos</p></div>
        </Link>
      </div>
    </div>
  )
